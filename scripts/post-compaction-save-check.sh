...
printf '{"hookSpecificOutput":{"hookEventName":"%s","additionalContext":"A context compaction happened earlier in this session while %s was still In Progress. The compaction summary may have dropped decisions or discoveries that only ever lived in conversation, never in a file — the raw pre-compaction transcript was backed up to %s as the only remaining copy of whatever that summary lost. Before continuing, run /workflow-dev:save on your own initiative to persist anything from before the compaction that is not in the story file yet — save still shows its usual summary and asks for confirmation before writing, this only skips waiting to be asked to run it. When presenting that confirmation this time, explicitly warn the human that declining leaves that information unrecovered — the backup gets deleted once a save actually runs (per save/SKILL.md Step 6), so it is not a copy that can be revisited later if this save is skipped now and the story moves on."}}' "$EVENT_NAME" "$STORY_FILE" "$BACKUP_FILE"
exit 0
